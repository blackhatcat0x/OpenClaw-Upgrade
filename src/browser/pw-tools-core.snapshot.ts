...
import { withPlaywrightContextRetry } from "./pw-tools-core.shared.js";
...
export async function resizeViewportViaPlaywright(opts: {
  ...
  // Make resize authoritative for persistent/CDP-attached sessions where
  // prior device emulation can keep a stale narrow viewport.
  const session = await page.context().newCDPSession(page);
  try {
    ...
    // Best effort: viewport size above is still applied.
  } finally {
    await session.detach().catch(() => {});
  }
}
